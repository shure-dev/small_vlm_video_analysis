...
ブラウザ上で「データセット → unit」を切り替えながら、複数unitを1プロセスで注釈できます。区間はタイムラインを**ドラッグ**して直接引け（端をつまんで伸縮も可）、開始/終了フレームのボタンや <kbd>i</kbd>/<kbd>o</kbd> でも指定できます。イベントの追加・削除、日本語ラベルや質問文、撮影状況のヒント（`domain_hint`）もその場で編集でき、SOP YAML へ検証付きで即書き戻されます。すべて自動保存です。詳細は [注釈ツールの仕様](docs/reference/annotator.md) を参照。
