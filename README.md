...
ブラウザ上で「データセット → unit」を切り替えながら、複数unitを1プロセスで注釈できます。区間はタイムラインを**ドラッグ**して直接引き、本体を掴んで移動・両端のつまみで伸縮できます（キーボード <kbd>i</kbd>/<kbd>o</kbd> でも指定可）。イベントの追加・削除、タイトルやVLMへ送る質問文、撮影状況のヒント（`domain_hint`）もカード上で直接編集でき、SOP YAML へ検証付きで即書き戻されます。すべて自動保存です。詳細は [注釈ツールの仕様](docs/reference/annotator.md) を参照。
