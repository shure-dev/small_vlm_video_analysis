...
- **Phase 1（observe）**: ローカルVLMが宣言済みの質問ごとにフレーム単位でyes/no/unclearを答える。トークンのlogitから実測した信頼度付き。
- **Phase 2（judge）**: 決定論的なルールエンジンが、観察結果を宣言済みの`events`と`relations`（`before`/`overlaps`/`not`）と突き合わせる。
