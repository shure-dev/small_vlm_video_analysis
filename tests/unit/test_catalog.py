"""unit台帳(catalog.discover)の回帰テスト。

tmp_pathに konro形(sop_refs=リスト+ground_truth_ref) と
factory形(sop_ref=dict、GT参照なし) の meta.json を置き、両スキーマを
1つの台帳へ吸収できることを検証する。実データにも依存しない。
"""
import json
...
def _make_tree(root: Path) -> None:
    ...
    _write(ku / "meta.json", {
        ...
        "sampling": {"fps": 1.0, "frame_path": "frames"},
        "sop_refs": ["../../sops/konro_inspection/konro_inspection.yaml"],
        "ground_truth_ref": "../../annotations/human-v001/konro_inspection.json",
    })
    (ku / "frames").mkdir(parents=True)
    for i in range(3):
        (ku / "frames" / f"f{i:03d}.jpg").write_bytes(b"x")
    # konroには既存GTがある
    _write(root / "datasets" / "konro_inspection" / "annotations" / "human-v001"
    ...
    _write(fu / "meta.json", {
        ...
        "sop_ref": {"id": "f001_w004", "path": "../../sops/f001_w004/v001.yaml"},
    })
    (fu / "frames").mkdir(parents=True)
    for i in range(2):
        (fu / "frames" / f"f{i:04d}.jpg").write_bytes(b"x")


def test_discover_absorbs_both_meta_schemas(tmp_path):
    _make_tree(tmp_path)
    ...
    # ground_truth_ref を尊重(規約パスに解決)
    assert konro.gt_path.parts[-3:] == ("annotations", "human-v001", "konro_inspection.json")

    fac = cat.get("f001_w004")
    assert fac.fps == 2.0
    assert fac.frame_files() == ["f0000.jpg", "f0001.jpg"]
    assert not fac.has_gt()                              # factoryはGT未作成
    # ground_truth_ref が無ければ規約パスを合成
    assert fac.gt_path.parts[-3:] == ("annotations", "human-v001", "f001_w004.json")
...
def test_summary_shape(tmp_path):
    ...
    assert s["konro_inspection"] == {
        ...
        "fps": 1.0, "has_frames": True, "has_gt": True,
    }
