...
| イベントカード | 質問文（=VLMプロンプト）をカード上で直接編集。下にタイムラインレーン |
...
イベントの追加・削除、質問文・撮影状況ヒントの編集は、その場でSOP YAMLへ検証付きに書き戻されます（`core.sop.save_sop`。`'yes'/'no'` は自動クォートされYAML 1.1のブール化は起きません）。

- **追加**: 「＋ イベントを追加」で空のカードが即増え、質問入力にフォーカスが移る（入力フォームは挟まない）。
- **削除**: カード内のインライン確認のうえ削除。`ground_truth.json` 内の同キーもカスケード削除して不整合を残さない。
- **編集**: 質問文はカード上で直接編集し、フォーカスを外すと保存。
